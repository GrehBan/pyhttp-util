...
class Headers:
    ...
    Attributes:
        ...
        allow_duplicates: Whether to bypass strict duplicate checking.
    """

    headers: list[Header] = field(default_factory=list)
    headers_map: CIMultiDict[str] = field(default_factory=CIMultiDict)
            ...
            if name_lower not in RFC7230Validator.ALLOWED_DUPLICATES:
                if name_lower in RFC7230Validator.COMMA_SEPARATED_HEADERS:
                    raise ValidationError(
                        f"Duplicate header '{header.name}' should be "
                        "combined with comma separation instead of "
                        "multiple header fields"
                    )
                raise ValidationError(
                    f"Duplicate header field not allowed: '{header.name}'"
                )
    ...
    def to_dict(self) -> CIMultiDict[str]:
