...
class BasicAuth:
    ...
    @classmethod
    def decode(cls, header: HeaderType, encoding: str = DEFAULT_ENCODING) -> "BasicAuth":
        ...
        if auth_type.lower() != "basic":
            raise ValueError(f"Authentication type must be Basic, got: '{auth_type}'")
        ...
        if ":" not in decoded_str:
            raise ValueError("Invalid credentials format: missing colon separator")
    ...
    def encode(self) -> str:
        ...
        return "Basic %s" % encoded_bytes.decode(self.encoding)
